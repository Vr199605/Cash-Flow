...
def render(df_raw, df_depara_raw, meses_sel: list, empresas_selecionadas: list):
    ...
    # Filtros de Setor e Departamento
    ...
    # Novo Filtro por Canal (Partner, B2B, B2C)
    canais_disponiveis = sorted(df_merged['Canal'].unique()) if 'Canal' in df_merged.columns else []
    if canais_disponiveis:
        canais_sel = st.multiselect("📡 Filtrar por Canal:", options=canais_disponiveis, default=canais_disponiveis)
    else:
        canais_sel = []

    # Aplicação dos filtros incluindo Canal
    df_analise = df_merged[
        df_merged['SETOR'].isin(setor_sel) & 
        df_merged['DEPARTAMENTO'].isin(depto_sel)
    ]
    
    if 'Canal' in df_analise.columns and canais_sel:
        df_analise = df_analise[df_analise['Canal'].isin(canais_sel)]
    ...
    st.subheader("📜 Detalhamento dos Custos")
     ...
     col_list = ['Data de pagamento', 'Mes_Ano', 'Categoria', 'SETOR', 'DEPARTAMENTO']
     if 'Canal' in df_analise.columns:
         col_list.append('Canal')
