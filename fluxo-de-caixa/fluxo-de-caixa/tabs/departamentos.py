...
def render(df_raw, df_depara_raw, meses_sel: list, empresas_selecionadas: list):
    ...
    # --- CORREÇÃO AQUI ---
    # Selecionamos apenas as colunas necessárias e removemos duplicatas de Centro de Custo
    # Isso garante que cada CC apareça apenas uma vez no merge, evitando multiplicar os valores.
    depara = df_depara_raw[['Centro de Custo', 'SETOR', 'DEPARTAMENTO']].copy()
    depara['CC_CLEAN'] = depara['Centro de Custo'].apply(_clean_cc)
    depara = depara.drop_duplicates(subset=['CC_CLEAN']) 
    # ---------------------
