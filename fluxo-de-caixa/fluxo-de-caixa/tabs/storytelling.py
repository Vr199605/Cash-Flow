...
def render(df, df_rec, df_geral, saidas_df, meses_sel):
    ...
    for col, label, val, color in zip([c1, c2, c3, c4], 
                                     ["RECEITA TOTAL", "DESPESA TOTAL", "RESULTADO LÍQUIDO", "CUSTO / RECEITA"],
                                     [rec_total, desp_total, res_liquido, indice_custo],
                                     ["#00ff00", "#ff4b4b", "#555", "#f1c40f"]):
        text_val = f"{val:.1f}%" if label == "CUSTO / RECEITA" else format_brl(val)
        col.markdown(f"""<div style="border: 1px solid {color}; padding: 15px; border-radius: 10px;">
                     <p style="margin:0; font-size: 10px; color: {color};">● {label}</p>
                     <h3 style="margin:0; font-size: 18px;">{text_val}</h3></div>""", unsafe_allow_html=True)

    # --- 3. EVOLUÇÃO DO FLUXO DE CAIXA (INTERATIVO) ---
    ...
    # --- 4. ANÁLISE DE PARETO (INTERATIVO) ---
    ...
    with col_est:
        ...
        c_est1.markdown(f'<div style="border: 1px solid #00D1FF; padding:10px; border-radius:10px;">'
                        f'<p style="font-size:10px; margin:0;">RECORRENTES</p>'
                        f'<b>{format_brl(v_recorrente)}</b></div>', unsafe_allow_html=True)
        c_est2.markdown(f'<div style="border: 1px solid #f1c40f; padding:10px; border-radius:10px;">'
                        f'<p style="font-size:10px; margin:0;">PONTUAIS</p>'
                        f'<b>{format_brl(v_pontual)}</b></div>', unsafe_allow_html=True)
    ...
    st.markdown(f"""<div style="background-color: #1a1c23; padding: 20px; border-radius: 10px; border-left: 5px solid #ff4b4b; margin-top:20px;">
                No período de {', '.join(meses_sel)}, a operação gerou <span style="color:#00ff00;">{format_brl(rec_total)}</span> em receitas 
                e consumiu <span style="color:#ff4b4b;">{format_brl(desp_total)}</span> em despesas, 
                resultando em um <span style="color:#ff4b4b;">déficit de {format_brl(res_liquido)}</span>.</div>""", unsafe_allow_html=True)
