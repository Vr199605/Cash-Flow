...
def exportar_pdf_premium(rec_total, desp_total, res_liquido, perc_conc, prev, top_5_rec, fig_flow, fig_p, meses_sel):
    ...
    pdf.cell(0, 5, f"Periodo: {', '.join(meses_sel)} | Emissao: 15/04/2026", ln=True, align='L')
    pdf.ln(10)

    # Cards de KPI (Verde e Vermelho)
    ...
    pdf.ln(35)
    try:
        # Nota: Necessário pacote 'kaleido' para as imagens funcionarem
        ...
        pdf.set_text_color(150, 150, 150); pdf.cell(0, 10, "(Grafico processado no sistema)", ln=True)
...
def render(df, df_rec, df_geral, saidas_df, meses_sel):
    ...
    # Preparação dos gráficos para o PDF antes da renderização
    ...
    fig_flow.add_trace(go.Scatter(x=df_m['Mes_Ano'], y=df_m['Saldo'], name='Saldo', line=dict(color='#00D1FF', width=3)))
    ...
    with col_s2:
        ...
        st.write("Recuo em margem líquida, variação de custos e concentração de receita detectados.")
    ...
    with c_rec1:
        with st.container(border=True):
            ...
            for (nome, valor), cor in zip(top_5_rec.items(), bar_colors):
                ...
                col_t, col_v_ui = st.columns([2, 1])
                col_t.caption(f"{nome}")
                col_v_ui.markdown(f"<p style='font-size:11px; text-align:right; margin:0;'>{format_brl(valor)} ({p:.1f}%)</p>", unsafe_allow_html=True)
                st.markdown(f"""<div style="background-color: #333; height: 6px; width: 100%; border-radius: 5px;">
                            <div style="background-color: {cor}; height: 6px; width: {min(p, 100)}%; border-radius: 5px;"></div></div><br>""", unsafe_allow_html=True)
    with c_rec2:
    ...
    # --- 7. ESTRUTURA DE CUSTOS (COM SETINHAS) ---
    ...
    st.markdown(f"""<div style="background-color: #1a1c23; padding: 20px; border-radius: 10px; border-left: 5px solid #ff4b4b; margin-top:20px;">
                ...
                Resultado líquido de <span style="color:#e74c3c;">{format_brl(abs(res_liquido))}</span> no período.</div>""", unsafe_allow_html=True)
